...
Link to blast:(https://blast.ncbi.nlm.nih.gov/Blast.cgi#alnHdr_1282621605)
...
In a phylogenetic tree, vertical branch lengths represent the genetic distance or substitution rate between sequences. Samples 5 and 6 are the most closely related; these two samples are the most closely related in the entire set of samples. They share a very recent common ancestor, indicated by the shortest horizontal distance between their branching point and the tips, samples 1 and 3 are highly related this pair forms a separate cluster of high similarity. While they are slightly more divergent from each other than the 5 and 6 samples pair, they are significantly more related to each other than to any other samples in the dataset. The clustered groups were 2,5, and 6 and 1,3, and 4. Sample 2 shares a common ancestor with 5 and 6 more recently than it does with the other three samples. Sample 4 is the outgroup of the cluster because it is more distinct from 1 and 3. Based on the tree structure and the historical data based on the Zaire ebolavirus isolate likely has a number of one individual because of the background information which includes the Kikwit-9510621 isolate was originated from a single 65 year old female patient during the outbreak in 1995. The tree showed differences by the genetic variation observed between these 6 samples, which is typically the result of viral passaging or internal host evolution. When a virus is grown in a lab through different cell lines or used in different experimental groups, it acquires mutations. In the data there are 6 samples but they most likely represent different versions or replicates of the same original human isolate used in various lab studies and trials meaning they all trace back to the same single human source. 
